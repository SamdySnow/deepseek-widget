...
> 最后更新：2026-09-20（第六轮：开机自启动 + 单实例互斥）
...
| 自动化验证 | ✅ `./Scripts/verify.sh` — **304 项全通过，0 失败** |
| 打包产物 | ✅ `dist/WhaleWidget.app`（8.5 MB） |
...
./Scripts/autostart.sh              # 开机自启动状态（on / off 也可）
./Scripts/count_checks.sh           # 统计各入口断言数（更新本文档用）
```
...
## 二·补五、第六轮工作：开机自启动 + 单实例互斥

用户要求：**在菜单里加入可控制开机自启动的开关，并真正实现开机自启动**。

### 24. 为什么用 LaunchAgent 而不是 `SMAppService` ✅

`AutoLaunch.swift`。三种候选方案都试过取舍：

| 方案 | 为什么不用 |
|---|---|
| `SMAppService.mainApp`（macOS 13+） | 要求**已签名**的 .app；本项目是 ad-hoc 签名，且常以裸二进制（`.build/release/WhaleWidget`）运行 |
| `SMLoginItemSetEnabled` | 需要在 .app 里额外塞一个 Helper bundle |
| `LSSharedFileList` | 已废弃，新系统行为不可靠 |

最终用 `~/Library/LaunchAgents/<label>.plist`：对「打包 .app」与「裸二进制」两种形态都有效，
用户能看到、能删。plist 的三个关键键：

- `RunAtLoad = true` —— 登录时启动；
- `KeepAlive = false` —— **不要**常驻拉起。设成 true 的话，用户在菜单里「退出」
  会被 launchd 立刻拉回来，变成一个**关不掉**的程序；
- `LimitLoadToSessionType = Aqua` —— 只在图形登录会话启动，避免 ssh 里也被拉起。

### 25. 状态的真值 = 那个 plist 文件（不写进 config.json）✅

用户可能在「系统设置 → 登录项」或终端里改掉它。只有**文件本身**不会说谎，
所以 `AutoLaunch.status()` 每次都去读盘解析 plist，不缓存、不落进 `config.json`。

三种状态，第三种是这轮最有价值的产出：

| 状态 | 含义 | 界面 |
|---|---|---|
| `.off` | 未登记（文件不存在，或那个 plist 不是本作业的 Label） | 开关关闭 + 说明 |
| `.on` | 已登记且指向**当前**可执行文件 | 开关打开 + 显示登记路径 |
| `.stale` | 已登记，但指向**别的**路径 | ⚠️ 报警 + 「改为当前程序」按钮 |

`.stale` 为什么必须单独处理：二进制被重新构建 / 移动过之后（例如从
`.build/release` 搬到 `dist/*.app`），plist 还指着旧位置。此时**开机启动的是那个旧程序**。
若不提示，用户看到的是「开关是开的」，直到重启后才发现没生效，而且没有任何线索。

界面上 `.stale` 的开关显示为**开**（`isRegistered`，不是 `isOn`）—— 已登记就该显示已登记；
若按 `isOn` 显示成「关」，用户一拨开关反而会去「开启」一个本来已经开着的自启动。

### 26. 单实例互斥（做完自启动才发现是必需的）✅

`SingleInstance.swift`。加了自启动之后，「同时跑两份」从边缘情况变成**常态**：
登录时 launchd 拉起一份，用户又双击了 `.app`（实测确实会）。
两份挂件的后果不是「多一个图标」：两个悬浮窗叠着、**各按自己的账本记账 → 余额读数互相打架**、
请求翻倍、退出只关掉一个（看起来像「退不掉」）。

用 flock 文件锁（`/tmp/whale-widget-<uid>.lock`）而不是 pid 文件：
锁的持有者是**进程**，无论怎么消失（正常退出 / 被 kill / 崩溃）内核都会释放，
不会留下僵尸锁把后续启动全挡死。第二份实例把已有实例带到最前然后退出（**退出码 0**，
不是错误）。

### 27. 本轮踩的坑（都是实测出来的）

**坑 A：同一进程重复 `flock` 同一个文件会失败。**
`--selftest` 里连调两次 `acquire()`，第二次拿到了**另一条** open file description，
`flock` 把它当成另一个竞争者而拒绝 → 「自己和自己抢锁」失败，且只在重复调用时出现。
修法：记住已持有的路径，同路径直接返回成功。这条有专门的回归断言。

**坑 B：`O_CLOEXEC` 不能省。**
不加的话子进程继承同一个 fd，父子共享同一条 open file description，
子进程再 `flock` 会**假成功** —— 跨进程互斥形同虚设。

**坑 C（最有价值）：`Toggle` 自身就画强调色像素，导致像素断言恒假红。**
`--render` 里「统计整节橙色像素判断有没有警告」这条判据**从根上就是错的**：
离屏渲染下 `Toggle` 的开关会画出一个橙红色块，而且**开 / 关两种状态分毫不差**
（实测都是恰好 784 个像素、位置固定在开关那一行）。
于是「本来就没有警告」的状态也数出同样的橙色 → 「不该有警告」的断言必然红。

修法不是调阈值（那只是把红灯涂绿），而是**把提示区从开关里拆出来**：
新增 `AutoLaunchNotice`（纯提示，不含开关），像素断言只对它做。
拆分后信号干净了 —— 非警告状态是 **0 个橙色像素**，警告状态是 10839 / 2323。
另外补了一条**对照断言**把「开关自带强调色且开/关相同」这个事实钉住，
否则以后有人把断言改回整节统计，就会重新踩这个坑。

**坑 D：两个界面显示同一份状态 → 会各显示一套。**
挂件菜单与菜单栏 🐋 各有开关，两处都是「用的时候才读文件」。
设置窗口开着时从菜单栏改一下，设置窗口会一直显示旧状态（看起来像开关失灵）。
修法：`AutoLaunch.didChangeNotification`，改动后广播、各处重新读盘。
（失败时也广播：删不掉的情况下文件还在，界面必须**如实**显示为「仍已登记」。）

### 28. 测试隔离（延续既有规矩）✅

`AutoLaunch.directory` 与 `logDirectory` 在 `AppConfig.isTestRun` 时切到临时目录，
**绝不动用户真实的登录项**（跑一次自检不该让用户多出一个自启动）。
另外 `loadIntoLaunchd` / `unloadFromLaunchd` 在自检下直接跳过 —— 测试不该去动
用户真实的 launchd。

验证过：真实 `~/Library/LaunchAgents` 里没有本作业的文件，
临时隔离目录也无残留。

### 29. 命令行入口 ✅

`--autostart [on|off|status]`（`AutoLaunchCommand.swift`）+ `Scripts/autostart.sh`。
**刻意不走隔离目录** —— 它要操作的就是用户真实的登录项（所以是显式命令，
不做任何「猜你想开」的自动化）。退出码区分开，脚本能据此判断：
...
端到端验证过完整链路：`on` → plist 落盘 → `launchctl print` 认到（`state = running`）
→ `off` → 文件与 launchd 登记都清干净。
...
├── Assets.swift                资源定位
├── AutoLaunch.swift            开机自启动（LaunchAgent plist 的读写 / 状态判定）
├── AutoLaunchCommand.swift      --autostart 命令行入口
├── AutoLaunchSection.swift     菜单里「开机自启动」一节（开关 + 提示区，可离屏断言）
├── SingleInstance.swift        flock 单实例锁（自启动 + 手动双击的互斥）
...
| 入口 | 覆盖内容 | 项数 |
|---|---|---|
| `--selftest` | 凭据解析 / 峰谷判定（含节假日边界、跨周末与跨长假倒计时）/ 计价 / 记账语义 / 定点精度 / DSH 历史导入 / 窗口定位 / 拖动跟随 / **吸附区宽度与四边独立** / **开机自启动（plist 内容 / 状态判定 / 「指向旧位置」/ 幂等 / 隔离）** / **单实例锁（含跨进程对照）** / 资源 / **真实联网拉余额** | 109 |
| `--hitcheck` | 命中遮罩逐像素准确性（只含角色本体）/ **click-through 路由** / **锁定语义** / **不透明度夹取** / **旧配置向后兼容** / **临时提醒与点击的关系** / 点击拖动判定 / 点击序列两种模式 / 菜单按钮独立性 | 95 |
| `--render` | 离屏渲染 + 像素断言（气泡填充/描边/文字、布局方位、文字不溢出气泡、逐页可渲染、**锁定角标**、**不透明度像素效果**、**镜像是「只翻朝向不翻内容」**、**开机自启动三种状态（提示区拆出后橙色 = 警告的干净信号）**) | 50 |
...
- [ ] **开机自启动的登记对象**：菜单里拨开关时登记的是**当前正在运行的可执行文件**。
      若你平时用 `dist/WhaleWidget.app` 双击启动、偶尔又用 `.build/release/WhaleWidget`
      跑开发版，两者会互相把对方登记成「旧位置」（界面会提示，点「改为当前程序」即可）。
      若要固定登记 `.app`，改 `AutoLaunch.executablePath` 的取值来源。
- [ ] **重启验证一次真机自启动**：`./Scripts/autostart.sh on` 后注销 / 重启，
      确认登录后挂件自动出现（自动化只能验到「plist 落盘 + launchctl 认到」，
      「登录时真的被拉起」这一步无法在无头环境合成）。
...
**flock / launchd / 像素断言（第六轮新增）**

- **同一进程重复 `flock` 同一个文件会失败**：第二次 `open()` 得到的是**另一条**
  open file description，`flock` 把它当成竞争者。必须记住「已持有的路径」，
  同路径直接返回成功。这个 bug 只在重复调用时出现，很容易漏测。
- **`O_CLOEXEC` 不能省**：否则子进程继承同一个 fd、共享同一条 open file
  description，子进程再 `flock` 会**假成功** —— 跨进程互斥形同虚设。
- **`KeepAlive` 必须为 `false`**：设成 true 的话用户在菜单里「退出」会被 launchd
  立刻拉回来，变成一个**关不掉**的程序。
- **`Toggle` 自身就画强调色像素，而且开 / 关数值完全相同**（实测都是 784 px、
  固定在同一行）。**「统计整块区域的橙色像素」这种判据从根上就是错的** ——
  没有警告的状态也会数出同样的橙色，断言必然假红。
  修法是把提示区从开关里**拆出来**单独渲染，而不是调阈值（调阈值只是把红灯涂绿）。
  并补一条对照断言把「开关自带强调色且开/关相同」钉住，防止有人改回整节统计。
- **同一份状态被两个界面显示时，光有「读的时候才取真值」还不够**：
  两个窗口同时开着就会各显示一套。要广播变更（本项目的
  `AutoLaunch.didChangeNotification`），并让失败路径也广播 ——
  删不掉时文件还在，界面必须如实显示为「仍已登记」。
