...
> 最后更新：2026-09-19 17:46（交互区域收窄到角色本体 + click-through + 点击序列修复）
...
| 自动化验证 | ✅ `./Scripts/verify.sh` — **180 项全通过，0 失败** |
| 打包产物 | ✅ `dist/WhaleWidget.app`（8.2 MB） |
...
| `--hitcheck` | 命中遮罩逐像素准确性（只含角色本体）/ **click-through 路由** / 点击拖动判定 / 点击序列两种模式 / 菜单按钮独立性 | 63 |
| `--render` | 离屏渲染 + 像素断言（气泡填充/描边/文字、布局方位、文字不溢出气泡、逐页可渲染） | 27 |
| `--e2e` | **起真实窗口 + 合成鼠标事件**走完整链路（点击序列、**同点连点状态轨迹**、拖动、菜单按钮冲突、透明区不响应） | 15 |
